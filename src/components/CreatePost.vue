...
const addTodo = async () => {
  if (newTodo.title.trim() === "" || newTodo.text.trim() === "") {
    ...
    error.value = false;
    const data = { collection: choosenCollection.value, todo: newTodo };
...
<template>
  <div class="px-5 w-5/6 mx-auto rounded py-2 mt-10 bg-slate-500">
    ...
    <div>
      <select
        v-model="choosenCollection"
        class="text-black capitalize w-1/6 outline-none"
      >
        <option
    ...
    <div>
      <button
        @click="addTodo"
        class="uppercase mt-8 mb-4 px-3 py-1 border-2 border-white rounded-sm transition hover:bg-white hover:text-black"
      >
        post
      </button>
    </div>
